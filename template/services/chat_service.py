    ...
    async def stream_chat(self, request:ChatRequest) -> StreamingResponse:
        ...
        async def event_stream() -> AsyncGenerator[str, None]:
            async with self._semaphore:
                state = await self._cache.load_state(request.session_id)
                ...
                if not state.messages and request.history:
                    ...
                    await self._cache.save_state(request.session_id, state)
                ...
                async for event in self._graph.astream_events(state, config, version = "v2"):
                        ...
                        if node_name in ["generate"]:
                            ...
                            if isinstance(output, ChatState):
                                ...
                                if current_status and current_status != last_status:
                                    status_payload = {"type": "status", "state": current_status}
                    ...
                    ref_payload = {"source": [title['source'] for title in final_chat_state.references],
                                   "page": [page['page'] for page in final_chat_state.references]}
                    yield f"data: {json.dumps(ref_payload, ensure_ascii = False)}\n\n"

                    if final_chat_state is not None:
                        await self._cache.save_state(request.session_id, final_chat_state)
