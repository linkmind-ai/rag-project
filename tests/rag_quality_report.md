...
| 측정일 | 프롬프트 버전 | Faithfulness | Answer Relevancy | 비고 |
|--------|------------|-------------:|-----------------:|------|
| 2026-03-12 | v1 (우선적 활용) | 61.6% ❌ | 78.7% ✅ | 초기 측정 |
| 2026-03-12 | v2 (엄격 금지) | **85.4% ✅** | **81.5% ✅** | 프롬프트 강화 후 |

> **v2 변경 내용**: "우선적으로 활용" → "컨텍스트 내용만 사용, 사전 학습 지식 사용 금지" 명시

### 최신 결과 (v2 프롬프트 기준)

| 지표 | 결과 | 기준 | 판정 |
|------|-----:|-----:|:----:|
| Faithfulness | **85.4%** | ≥ 70% | ✅ 합격 |
| Answer Relevancy | **81.5%** | ≥ 70% | ✅ 합격 |

### v1 → v2 개선 분석

**v1 Faithfulness 미달 패턴 (컨텍스트 이탈 사례)**
...
**v2 개선 효과**: "배경 지식이나 추론을 추가하지 마세요" 명시적 금지 지시로 +23.8%p 향상
...
### 7-1. ~~[단기 / 높음] 프롬프트 강화로 Faithfulness 개선~~ ✅ 완료

**결과**: Faithfulness 61.6% → **85.4%** (+23.8%p)

**적용 내용 (`apps/prompts/chat_prompt.py`, `tests/test_ragas.py`)**:
```
# 변경 전
컨텍스트에 있는 정보를 우선적으로 활용하세요.

# 변경 후
반드시 위 컨텍스트에 포함된 내용만을 사용하여 답변하세요.
컨텍스트에 없는 내용은 절대 추가하지 마세요. 사전 학습된 지식을 사용하지 마세요.
컨텍스트에서 답을 찾을 수 없다면 "제공된 문서에서 해당 정보를 찾을 수 없습니다"라고 답하세요.
```
...
```
완료
└── 7-1. 프롬프트 강화       ✅ Faithfulness 85.4% 달성 (2026-03-12)

단기 (2~4주)
├── 7-3. 평가-서비스 k 통일  → TOP_K_RESULTS=5 or 평가 k=3
└── 7-2. 청크 크기 실험       → 재인덱싱 필요 (CHUNK_SIZE=500 테스트)
