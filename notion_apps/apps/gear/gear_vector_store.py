...
class ElasticsearchStore:
    ...
    async def _create_index(self) -> None:
        ...
        index_exists = await self._es_client.indices.exists(index=settings.ELASTICSEARCH_INDEX)
        ...
        if not self.client.indices.exists(index=self.triples_index_name):
            self._es_client.indices.create(
                index=self.triples_index_name,
                body={
                    "mappings": {
                        "properties": {
                            "doc_id": {"type": "keyword"},
        ...
        for doc in documents:
            ...
            chunk_metadata = {
                **doc.metadata,
                ...
                "origin_doc_id": doc.doc_id or self._generate_doc_id(doc.content, doc.metadata)
            }
    ...
    async def hybrid_search(
        ...
        max_keyword_score = max(keyword_results.scores) if keyword_results.scores else 1.0
        for doc, score in zip(keyword_results.documents, keyword_results.scores):
            ...
            if doc.doc_id in doc_scores:
                ...
                doc_scores[doc.doc_id] = (existing_doc, existing_score + normalized_score * (1 - vector_weight))
            else:
    ...
    async def _index_document_with_triples(
            self,
            doc_id: str,
            content: str,
