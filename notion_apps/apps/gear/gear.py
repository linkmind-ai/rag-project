...
class GearRetriever:
    ...
    async def _extract_initial_triples(self) -> None:
        ...
        try:
            ...
            self._initial_triples_cache = await self._extract_triples_from_docs(all_docs)
    ...
    def rrf_fusion(self, rank_lists: List[List[Document]], k: int = 60) -> List[Document]:
        """RFF 알고리즘(랭크 역수 연산)"""
    ...
    def _word_matching_score(
            self,
            triple1: Dict[str, Any],
            triple2: Dict[str, Any]
    ) -> float:
    ...
    async def triple_link(
            self,
            proximal_triples: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """
    ...
    async def graph_expansion(
        ...
        for exp_triple in expanded_triples:
            ...
            query=exp_triple, # 여기에 사용자 질의도 더해본다면 어떨까?
            k=max_docs
            )
    ...
    async def reason(
        ...
        try:
            result = await asyncio.to_thread(
                chain.invoke,
                {"query": query, "triples": triples_text}
            )
