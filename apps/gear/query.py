    ...
    async with request_semaphore:
        ...
        try:
            ...
            sources = [
                ...
                for idx, doc in zip(response["evidence_indices"], response["evidence_docs"])
            ]
...
@router.post("/gear", response_model=QueryResponse)
async def query_with_gear(request: QueryRequest):
    """GeAR 기반 질의응답 엔드포인트"""
    async with request_semaphore:
        start_time = time.time()

        try:
            documents, gist_memory = await gear_retriever.gear_retrive(
                query=request.query,
                max_steps=settings.
            )