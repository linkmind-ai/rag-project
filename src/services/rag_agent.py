...
class RagAgent:
    ...
    def query(self, question: str) -> str | None:
        ...
        logger.info(f"--- 1. HyDE: '{question}'에 대한 가상 답변 생성 중...")
        ...
        if not expanded_query:
            logger.warning("⚠️ HyDE 답변 생성 실패. 원본 질문으로 검색합니다.")
            expanded_query = question

        logger.info("--- 2. Search: HyDE 답변으로 ES k-NN 검색 중...")
        contexts_docs = self.vector_store.search_knn(expanded_query, k=3)

        if not contexts_docs:
            logger.warning("❌ Elasticsearch에서 컨텍스트를 찾지 못했습니다.")
            return None

        logger.info(f"--- 3. RAG: 검색된 컨텍스트(x{len(contexts_docs)})로 최종 답변 생성 중...")
